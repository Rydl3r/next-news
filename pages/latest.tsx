...
const latest = () => {
  ...
  const fetchData = () => {
    axios
      .get(
        `https://api.newscatcherapi.com/v2/search?q=Programming&lang=en&page=${page}&page_size=20`,
        {
          headers: {
  ...
  return (
    <div className="text-center py-5">
      <h1 className="font-bold text-4xl pb-5">
        Latest news on programming world
      </h1>
      <div className="flex flex-wrap justify-around">
        {!posts
          ? "No news were found! We're sorry..."
          : posts.map((post) => {
              return (
                <div className="bg-gray-200 text-center max-w-sm w-auto rounded-lg my-5 pb-10">
                  <div className="h-48 w-96 pb-5">
                    <img
                      className="max-w-sm min-w-full rounded-t-lg max-h-48 min-h-full"
                      src={
                        post.media
                          ? post.media
                  ...
                  <div className="px-5 pb-5">{post.summary}</div>
                  <Link
                    href={
                      post && post.summary && post.summary.length > 200
